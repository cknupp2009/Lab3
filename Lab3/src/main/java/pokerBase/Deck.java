...
public class Deck {
	...
	public JokerDeck(){
		// Creating an ArrayList of Joker Cards,
		ArrayList<Card> MakingJokerDeck = new ArrayList<Card>();
		for (short a = 0; a<=4; a++){
			eRank RankValue = eRank.JOKER;
			eSuit SuitValue = eSuit.JOKER;
			Card NewCard = new Card(SuitValue, RankValue, a++);
			MakingJokerDeck.add(NewCard);
			
		}
		cards = MakingJokerDeck;
		ShuffleCards();
	}
	public DeckWithJokers(){
		ArrayList<Deck> = Deck.add(JokerDeck);
		ShuffleCards();
	}
